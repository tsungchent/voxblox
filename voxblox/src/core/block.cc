...
void serializeDirection(const Eigen::Vector3i& parent_direction,
  ...
  const int8_t parent_direction_x =
      std::min<int>(INT8_MAX, std::max(parent_direction.x(), INT8_MIN));
  const int8_t parent_direction_y =
      std::min<int>(INT8_MAX, std::max(parent_direction.y(), INT8_MIN));
  const int8_t parent_direction_z =
      std::min<int>(INT8_MAX, std::max(parent_direction.z(), INT8_MIN));
